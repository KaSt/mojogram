function MystatusAssistant(){
    this.statusChoices = Utilities.statusChoices;
}
...
MystatusAssistant.prototype.saveChangesHandler = function(event){
    ...
    Mojo.Log.info("status current %s", status);
...
MystatusAssistant.prototype.setCurrentStatusTitle = function(subject){
    ...
    this.controller.get('currentStatusTitle').update($L("Your current status") + " (" +
    $L("#{left} remaining").interpolate({
        left: left
    }) +
    ")");
}
