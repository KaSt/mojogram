...
MainAssistant.prototype.updateBroadcasts = function(){
    var chats = [];
    _appDB.getAllChatsWithMessages(function(chats){
        this.mainListModel.items[1].size = chats.length;
        this.mainListModel.items[1].chats = chats;
        ...
        if (event.index === 0) {
            ...
            if (event.index === 3) {
                ...
                if (event.index === 4) 
                    ...
                    if (event.index === 1) 
                        this.controller.stageController.pushScene("chats-list", event.item.chats, true);
}
...
MainAssistant.prototype.updateMyStatus = function(){
    this.mainListModel.items[3].status = _appData.cookieData.statusMessage;
    this.controller.modelChanged(this.mainListModel);
}
