    ...
    this.controller.setupWidget("messageList", {
        ...
        formatters: {
            ...
            msgType: function(value, model){
                if ("data" in model && model.status == Message.STATUS_NEW_MESSAGES) 
                    return "newmessages";
...
.bind(this)            ,
    ...
    if (this.model.value != "") {
        this.menuModel.items[this.sendIndex].icon = "send";
        this.menuModel.items[this.sendIndex].command = "send";
...
ChatAssistant.prototype.handleScrollHold = function(event){
    if (event.type == Mojo.Event.dragStart) {
        ...
        if (event.type == Mojo.Event.dragEnd) {
            this.scrollHolded = false;
        }
}
...
ChatAssistant.prototype.buildAppMenuItems = function(){
    ...
    // },
    {
        label: $L("Export messages to file"),
...
.bind(this);
    ...
    if (this.chat.unread > 0) {
        _appDB.countMessagesFromFirstUnread(this.chat, function(count){
            this.messagesToLoad = count + this.INCREMENT_LOAD;
            this.loadMessages(this.messagesToLoad, onLoad);
        }
.bind(this));
    }
    else {
...
.bind(this), 2000);
    ...
    var text = String(this.messageTextWidget.mojo.getValue());
    
    if (text != "" && this.menuModel.items[this.sendIndex].command != "stopRecord") {
        this.menuModel.items[this.sendIndex].icon = "send";
        this.menuModel.items[this.sendIndex].command = "send";
        this.controller.modelChanged(this.menuModel);
    }
        ...
        if (text == "" && this.menuModel.items[this.sendIndex].command != "stopRecord") {
            this.menuModel.items[this.sendIndex].icon = "record";
            this.menuModel.items[this.sendIndex].command = "record";
            this.controller.modelChanged(this.menuModel);
        }
    
    _chatTextClipboard.setItem(this.chat.jid, text);
}
...
ChatAssistant.prototype.getPopupMenu = function(msg){
    ...
    if (this.chat.isGroup && !msg.from_me) {
        var info = this.nameDisplayHash.getItem(msg.remote_resource);
        if (info === undefined) {
            info = msg.notifyname;
        } else {
			info = info.name;
...
ChatAssistant.prototype.popupHandler = function(command){
    ...
    switch (command) {
		case 'chatWith':
			_appDB.findOrCreateChat(msg.remote_resource, function(chat) {
...
ChatAssistant.prototype.loadMessages = function(numrows, onLoad){
    _appDB.getFirstMessages(this.chat, numrows, function(messages){
        ...
        for (var i = 0; i < messages.length; i++) {
            if ((newMsgIndex == -1) && (this.chat.unread > 0) && !messages[i].from_me && !messages[i].isNotification() && (messages[i].timestamp >= this.chat.firstunread)) {
                newMsgIndex = i;
            }
        ...
        if ((newMsgIndex >= 0) && (this.chat.unread > 0)) {
            var newMsg = new Message("", "");
            newMsg.status = Message.STATUS_NEW_MESSAGES;
            newMsg.data = "" + this.chat.unread;
            newMsg.timestamp = this.chat.firstunread;
            messages.splice(newMsgIndex, 0, newMsg);
        }
        ...
        if (newMsgIndex >= 0) {
            this.messageListWidget.mojo.revealItem(newMsgIndex + 1, false);
        }
        else 
...
.bind(this));
            ...
            case "exit":
                ...
                break;
            case "exportmessages":
                var fileName = this.chat.chatName.replace(/\s+/g, "");
                fileName = fileName.replace(/\W/g, "");
                fileName = "Chat-" + fileName + "-" + Message.removeDomainFromJid(this.chat.jid) + ".html";
                _appDB.exportMessages(this.chat, Media.MOJOWHATSUP_MEDIA_DIR + fileName);
                break;
...
ChatAssistant.prototype.decreaseTimer = function(){
    this.recordDuration--;
    this.controller.get("timer-text").update($L("Recording") + ": " + this.recordDuration + " " + $L("seconds"));
    if (this.recordDuration == 0) {
        this.stopRecord();
    }
}

ChatAssistant.prototype.startRecord = function(){
    this.captureHelper = new CaptureHelper();
    Mojo.Log.info("CaptureHelper: " + this.captureHelper.initDeviceAndFormat(8000));
    var fileName = Media.MOJOWHATSUP_MEDIA_DIR + ".VoiceMemo-" + Media.getDateFormatted() + this.captureHelper.extension;
    this.captureHelper.filename = fileName;
    
    this.captureHelper.startRecording(fileName, function endedCapture(response){
        this.controller.window.clearInterval(this.timerRecord);
    }
.bind(this));
    this.recordDuration = this.VOICE_RECORD_DURATION;
    this.controller.get("timer-text").style.display = "block";
    this.decreaseTimer();
    this.menuModel.items[this.sendIndex] = {};
    this.controller.modelChanged(this.menuModel);
    this.timerRecord = this.controller.window.setInterval(this.decreaseTimer.bind(this), 1000);
...
ChatAssistant.prototype.stopRecord = function(){
    if (this.captureHelper != null) {
        ...
        this.controller.get("timer-text").style.display = "none";
        this.controller.modelChanged(this.menuModel);
    }
}

ChatAssistant.prototype.cancelRecord = function(){
    if (this.timerRecord) 
        this.controller.window.clearInterval(this.timerRecord);
    
    if (this.captureHelper != null) {
        this.captureHelper.cancelRecording();
        _mojowhatsupPlugin.safePluginCall(function(){
            _plugin.removeFile(this.captureHelper.filename);
        }
.bind(this));
    }
    ...
    _appDB.saveMessage(msg, function(msg){
        ...
        _appDB.updateChat(this.chat, function(updated){
            ...
            setTimeout(function(){
                ...
                Media.getThumbImage(this.controller.document, msg, function(thumbBase64){
                    msg.data = thumbBase64;
                    MediaUploader.upload(msg, file.fullPath);
                });
            }
.bind(this), 1000);
        }
...
ChatAssistant.prototype.activate = function(event){
    ...
    //            else 
    //                if (event && "forwardTo" in event) {
    //					this.forwardMessageTo(this.forwardMsg, event.forwardTo);
...
ChatAssistant.prototype.setBackgroundImage = function(){
    var backimagePath = PreferencesCookie.DEFAULT_BACKGROUND_IMAGE;
    if (_appPrefs.cookieData.backimageType == "custom") {
        backimagePath = _appPrefs.cookieData.backimagePath;
        this.controller.get("mojo-scene-chat-scene-scroller").style.background = "url('" + backimagePath + "') no-repeat center center !important;";
        this.controller.get("mojo-scene-chat-scene-scroller").style.backgroundSize = "cover !important";
    }
    else 
        if (_appPrefs.cookieData.backimageType == "none") {
            backimagePath = "";
            this.controller.get("mojo-scene-chat-scene-scroller").style.background = "none !important";
        }
        else {
            this.controller.get("mojo-scene-chat-scene-scroller").style.background = "url('" + backimagePath + "') repeat !important;";
        }
