...
var _mojoWhatsupService = new MojoWhatsupService();
var _broadcastAssistant = null;
            ...
            if (_mainStageController) {
                ...
                var onSuccess = function(controller){
                    ...
                    _mojowhatsupPlugin.whenReady(function(){
                        ...
                        if (sceneName == "chats-list" && launchParams) {
                            this.openChat(launchParams.chats);
                        }
                    }
.bind(this));
            ...
            if (launchParams.action == "dashboard") {
                ...
                if (launchParams.action == "timeout") {
                    if (launchParams.type == "foreground") {
                        ...
                        if (launchParams.type == "background") {
...
AppAssistant.prototype.openChat = function(chats){
    if (chats) {
        ...
        if (chats.length > 1) {
            ...
            _appDB.findChat(chats[0].jid, function(fChat){
                if (fChat) {
                    if (_openChatAssistant != null && _openChatAssistant.chat.jid != chats[0].jid) {
                        _mainStageController.swapScene("chat", fChat);
                    }
                    else 
                        if (_openChatAssistant != null) 
                            _mainStageController.activate();
                            ...
                            if (_chatsAssistant != null) 
                                _mainStageController.pushScene("chat", fChat);
                            else 
                                _mainStageController.pushScene("chat", fChat);
                }
            });
...
AppAssistant.prototype.notifyUpdateBroadcast = function(message){
    Mojo.Log.info("[AppAssistant]Received notifyBroadcastChats!")
    if (_mainAssistant != null) 
        _mainAssistant.updateBroadcasts();
    
    if (_chatsAssistant != null) 
        _chatsAssistant.updateChats();
    
    if (message && _broadcastAssistant != null) {
        if (_broadcastAssistant.params.action == "info" && _broadcastAssistant.params.chat.jid == message.remote_jid) {
            _broadcastAssistant.updateBroadcastStatus(message);
        }
    }
}
...
AppAssistant.prototype.alertChatExported = function(result){
    var bannerText = "";
    var icon = "";
    if (result == "ok") {
        icon = 'images/status-downloaded.png';
        bannerText = $L("Message history saved successfully!");
    }
    else {
        icon = 'images/status-error.png';
        bannerText = $L("Error saving message history!");
    }
...
AppAssistant.prototype.alertNewMessage = function(chat, msg){
    ...
    var bannerText = (name == null ? "" : name + ": " + msg.formatTextMessage(false, false, 0, true));
    
    var isGroup = chat.isGroup && !msg.isbroadcast;
    
    var currentDate = new Date().getTime();
    
    if (isGroup && chat.muteexpiry) {
        if (chat.muteexpiry > (currentDate / 1000)) 
            return;
    }
    ...
    if ((isGroup && _appPrefs.cookieData.groupSound != "silence") || (!isGroup && _appPrefs.cookieData.personSound != "silence")) {
        if ((isGroup && ((currentDate - _lastSoundGroupNotification) > _MINIMUM_NOTIFICATION_INTERVAL)) ||
        (!isGroup && ((currentDate - _lastSoundPersonNotification) > _MINIMUM_NOTIFICATION_INTERVAL))) {
            if ((isGroup && _appPrefs.cookieData.groupSound == "system") || (!isGroup && _appPrefs.cookieData.personSound == "system")) {
                this.controller.playSoundNotification("notifications");
            }
            else 
                if (isGroup && _appPrefs.cookieData.groupSound == "custom") {
                    this.controller.playSoundNotification("notifications", _appPrefs.cookieData.groupTonePath);
                }
                else 
                    if (!isGroup && _appPrefs.cookieData.personSound == "custom") {
    ...
    if ((isGroup && _appPrefs.cookieData.groupVibrate) || (!isGroup && _appPrefs.cookieData.personVibrate)) {
        if ((isGroup && ((currentDate - _lastVibrateGroupNotification) > _MINIMUM_NOTIFICATION_INTERVAL)) ||
        (!isGroup && ((currentDate - _lastVibratePersonNotification) > _MINIMUM_NOTIFICATION_INTERVAL))) {
    ...
    if ((isGroup && _appPrefs.cookieData.groupBanner) || (!isGroup && _appPrefs.cookieData.personBanner)) {
        this.controller.showBanner({
            messageText: bannerText,
    ...
    this.mwatchRequest = new Mojo.Service.Request('palm://com.palm.applicationManager', {
        ...
        parameters: {
            ...
            params: {
                command: "SMS",
                info: (isGroup ? "[" + chat.chatName + "]" : "") + bannerText,
                wordwrap: true
            }
        },
        onSuccess: function(){
    ...
    if ((isGroup && _appPrefs.cookieData.groupBlink) || (!chat.isGroup && _appPrefs.cookieData.personBlink)) {
...
AppAssistant.prototype.notifyContactInfoUpdated = function(contactInfo){
    ...
    if (_chatsAssistant != null) {
        _chatsAssistant.updateContactInfo(contactInfo);
    }
    
    if (_contactsAssistant != null) {
        _contactsAssistant.updateContactInfo(contactInfo);
    }
