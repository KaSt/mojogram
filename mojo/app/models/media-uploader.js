...
function MediaUploader(msg, file){
    this.msg = msg;
...
MediaUploader.notifyUpdateStatus = function(msg){
    if (_openChatAssistant != null) 
        _openChatAssistant.updateMessageStatus(msg);
    if (msg.remote_jid.indexOf("broadcast") == 0) {
        _appAssistant.notifyUpdateBroadcast(msg);
    }
    else {
        _appAssistant.notifyUpdateChats();
    }
}
...
MediaUploader.prototype.requestUpload = function(){
    ...
    _appDB.updateMessageStatus(this.msg, function(){
        MediaUploader.notifyUpdateStatus(this.msg);
    }
.bind(this));
        ...
        if (this.msg.media_url) {
            ...
            _appDB.updateMediaMessage(this.msg, function(){
                ...
                _mojowhatsupPlugin.safePluginCall(function(){
                    ...
                    if (this.msg.remote_jid.indexOf("broadcast") == 0) {
                        _appAssistant.notifyUpdateBroadcast(this.msg);
                        _plugin.sendBroadcastMessage(this.msg.remote_resource, this.msg.toJSONString());
                    }
                    else {
                        _appAssistant.notifyUpdateChats();
...
MediaUploader.upload = function(msg, file){
    if (_mediaUploads.getItem(msg.keyString()) !== undefined) {
        ...
        msg.mediaUploader = new MediaUploader(msg, file);
...
MediaUploader.stopUpload = function(msg){
    if (_mediaUploads.getItem(msg.keyString()) !== undefined) {
        ...
        _appDB.updateMessageStatus(msg, function(){
			MediaUploader.notifyUpdateStatus(msg);
        }
.bind(this));
    }
}
