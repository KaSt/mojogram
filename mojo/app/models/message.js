...
Message.STATUS_NEW_MESSAGES = 196;
...
Message.fromJSONString = function(messageString){
    ...
    if ('isbroadcast' in msgJson) 
        result.isbroadcast = msgJson.isbroadcast;
    
    return result;
}

Message.removeDomainFromJid = function(jid){
    var a = jid.replace(/@s\.whatsapp\.net/g, "");
    a = a.replace(/@g\.us/g, "");
    return a;
}
    ...
    switch (this.media_wa_type) {
        case Message.WA_TYPE_IMAGE:
            if (this.data != null && this.data != "") {
                return '<img class="preview-image image-custom" src="data:image/jpeg;base64,' + this.data + '"></img>';
            }
            else 
                if (this.downloadedFile != null && this.downloadedFile != undefined) {
                    var imageFile = (this.from_me ? '' : Media.MOJOWHATSUP_MEDIA_DIR) + this.downloadedFile;
                    var imgSize = "120:92";
                    if (_appAssistant.isPre3()) 
                        imgSize = "180:138";
                    return '<img class="preview-image image-custom" src="/var/luna/data/extractfs' + encodeURIComponent(imageFile) + ':0:0:' + imgSize + ':3"></img>';
                }
                else {
...
Message.prototype.formatTextMessage = function(newlinefyBool, emojifyBool, emojiSize, showType){
    if (this.isNotification()) {
        if (this.status == Message.STATUS_NEW_MESSAGES) {
            return '<img src="images/down-icon.png"/>' +
            $L("#{unread} unread messages").interpolate({
                unread: this.data
            }).toUpperCase() +
        ...
        switch (this.status) {
            ...
            case Message.STATUS_USER_ADDED:
                return $L("#{id} has joined the group").interpolate({
                    id: id
                });
                break;
            case Message.STATUS_USER_REMOVED:
                return $L("#{id} has left the group").interpolate({
                    id: id
                });
            case Message.STATUS_SUBJECT_CHANGED:
                var subject = this.data;
                return emojify($L("#{id} has changed the group subject by '#{subject}'").interpolate({
                    id: id,
                    subject: subject
                }), 16);
                break;
            case Message.STATUS_PICTURE_CHANGED_SET:
                return $L("#{id} updated the group picture").interpolate({
                    id: id
                });
                break;
            case Message.STATUS_PICTURE_CHANGED_DELETE:
                return $L("#{id} deleted the group picture").interpolate({
                    id: id
                });
                break;
        }
        ...
        if (this.media_wa_type == Message.WA_TYPE_UNDEFINED) {
            if (this.data) {
            ...
            var text = "";
            text += (showType ? "[" + $L(Message.getTypeString(this.media_wa_type)) + "] " : "");
            ...
            if (this.status == Message.STATUS_MEDIA_DOWNLOADING && this.mediaDownloader != null) {
                ...
                if (this.status == Message.STATUS_MEDIA_UPLOADING && this.mediaUploader != null) {
                    text += "<br>" + $L("Uploading") + ": " + Math.round(this.mediaUploader.progress) + "%";
                }
                else 
                    if (this.media_wa_type == Message.WA_TYPE_LOCATION) {
                        text += $L("Longitude") + ": " + this.longitude;
                        text += "<br> " + $L("Latitude") + ": " + this.latitude;
                    }
                    else 
                        if (this.media_wa_type == Message.WA_TYPE_CONTACT) {
                            text += this.media_name;
                        }
            if (!newlinefyBool) {
...
Message.prototype.isNotification = function(){
    return (this.status == Message.STATUS_NEW_MESSAGES) || (this.status == Message.STATUS_USER_ADDED || this.status == Message.STATUS_USER_REMOVED || this.status == Message.STATUS_SUBJECT_CHANGED || this.status == Message.STATUS_PICTURE_CHANGED_SET || this.status == Message.STATUS_PICTURE_CHANGED_DELETE);
}

Message.prototype.isForwardable = function(){
    return !this.isNotification() && !(this.status == Message.STATUS_MEDIA_DOWNLOADING || this.status == Message.STATUS_MEDIA_UPLOADING || this.status == Message.STATUS_MEDIA_UPLOADERROR || (this.status == Message.STATUS_NONE && this.from_me));
}
