...
MojowhatsupPluginModel.prototype.registerHandlers = function() {
	...
	_plugin.onSendGetPicture = function(jid, picturePath) {
		setTimeout(this.onSendGetPicture.bind(this), 1, jid, picturePath);
	}.bind(this);
...
MojowhatsupPluginModel.prototype.onSendGetPicture = function(jid, picturePath) {
	if ((jid != "error") && (picturePath != "")) {
		...
		_appDB.findChat(jid, function(chat) {
			...
			if (chat != null) {
				_appDB.updateChatPicture(chat.jid, chat.pictureid, picturePath, function() {
					this.updatePicture(jid, picturePath);
				}.bind(this));
			}
			this.processPictureIdsQueue();
		}.bind(this));
	} else {
...
MojowhatsupPluginModel.prototype.updatePicture = function(jid, picturepath) {
	...
	if (picturepath != "error") {
		if ((_openChatAssistant != null) && (_openChatAssistant.chat.jid == jid)) {
			_openChatAssistant.chat.picturepath = picturepath;
		}
...
MojowhatsupPluginModel.prototype.processPictureIdsQueue = function() {
	if (this.pictureIdsQueue.length == 0) {
		if ((_chatsAssistant != null) && (this.picturesChanged)) {
	...
	_appDB.findChat(item.jid, function(chat) {
		if (item.jid == _myJid) {
			...
			if (chat == null) {
				try {
					_plugin.sendGetPicture(_myJid, "image", _appData.cookieData.mypictureid, item.id);
				} catch (e) {
